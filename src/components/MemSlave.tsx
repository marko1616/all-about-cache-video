...
export class MemSlave extends Rect {
  ...
  @signal()
  public declare readonly highlightFill: ColorSignal<this>;

  @initial('#474747')
  @signal()
  public declare readonly writeHighlight: ColorSignal<this>;
        ...
        >
          ...
          >
            ...
            <Layout
              ...
              padding={[0, 40]}
            >
              <Txt fill={this.contentFill} fontSize={48} fontWeight={700} text={'ADDR'} />
              <Txt fill={this.contentFill} fontSize={48} fontWeight={700} text={'DATA'} />
            </Layout>
  ...
  private *processTransaction(payload: Payload<RequestPacket>): MotionGenerator<Payload<ResponsePacket>> {
    ...
    if (!this.isDetailedMode) {
      ...
      const rowCenterFromTop = (centerAddr * rowHeightWithMargin) + (rowHeightWithMargin / 2);
    ...
    if (isWriteRequest(payload.content)) {
      ...
      responseDisplay = `WR${sizeSuffix} [${addrHex}] ${valHex}`;
      ...
      if (this.isDetailedMode) {
        ...
        yield* all(
          ...affectedAddrs.map(addr =>
            this.rowValueTexts[addr].fill(this.writeHighlight(), 0.4).to(this.contentFill(), 0.4)
          )
        );
      }
    } else {
    ...
    if (this.isDetailedMode) {
      yield* all(
        ...
        ...affectedAddrs.map(addr =>
          this.rowValueTexts[addr].scale(1, 0.6)
        ),
      );
    }
