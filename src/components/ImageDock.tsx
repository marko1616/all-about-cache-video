...
function getDistributionPoints(count: number, start: number, end: number): number[] {
  if (count === 0) return [];
...
export class ImageDock extends Layout {
  ...
  public *focus(focusIndex: number, duration: number = 1.6) {
    ...
    const newDockPoints = getDistributionPoints(otherRefs.length, DOCK_RANGE[0], DOCK_RANGE[1]);
  ...
  public *exit() {
    const startPoints = getDistributionPoints(this.imageRefs.length, DOCK_RANGE[0], DOCK_RANGE[1]);
